...
export const AdminDashboardScreen: React.FC = () => {
  ...
  const handleCreateStreamer = () => {
    ...
    const newStreamer: Streamer = {
      ...
      email: streamerForm.email || undefined,
      avatar: streamerForm.avatar || "https://i.pravatar.cc/300?img=" + Math.floor(Math.random() * 70),
      headerImages: [
      ...
      schedule: [],
      followerCount: 0,
      referralCode: streamerForm.referralCode || "STREAM" + Math.random().toString(36).substring(2, 8).toUpperCase(),
      streamPlatforms: {
        twitch: streamerForm.twitchUrl || undefined,
        youtube: streamerForm.youtubeUrl || undefined,
        tiktok: streamerForm.tiktokUrl || undefined,
        instagram: streamerForm.instagramUrl || undefined,
      },
    };

    addStreamer(newStreamer);

    // Create streamer account if email and password provided
    if (streamerForm.email && streamerForm.password) {
      addStreamerAccount({
  ...
  const handleEditStreamer = () => {
    if (!selectedStreamer) return;

    updateStreamer(selectedStreamer.id, {
      name: streamerForm.name,
      gamertag: streamerForm.gamertag,
      avatar: streamerForm.avatar || selectedStreamer.avatar,
      bio: streamerForm.bio,
      referralCode: streamerForm.referralCode,
    });
  ...
  const handleDeleteStreamer = (id: string) => {
    deleteStreamer(id);
  };
  ...
  const handleToggleLive = (streamer: Streamer) => {
    const newLiveStatus = !streamer.isLive;
    const liveTitle = newLiveStatus ? `${streamer.name} is now streaming!` : undefined;

    // Update streamer live status
    ...
    if (newLiveStatus && liveTitle) {
      notifyFollowersGoLive(streamer.id, streamer.name, liveTitle);
    }

    // Create announcement for live status change
    if (user) {
      const message = newLiveStatus
        ? `🔴 ${streamer.name} is now LIVE! Check out the stream!`
        : `${streamer.name} has ended their stream. Thanks for watching!`;
      ...
      const announcement: Announcement = {
        ...
        duration: newLiveStatus ? 24 : 0.5, // Live announcements last 24hrs, offline 30min
        expiresAt: new Date(
          Date.now() + (newLiveStatus ? 24 : 0.5) * 60 * 60 * 1000
        ).toISOString(),
        createdAt: new Date().toISOString(),
        isActive: true,
      };
          ...
          <View className="px-4 pb-4">
              ...
              >
                ...
                {streamer.headerImages && streamer.headerImages.length > 0 && (
                  <View className="mb-3">
                    ...
                    <ScrollView horizontal showsHorizontalScrollIndicator={false}>
                      {streamer.headerImages.map((img, idx) => (
                        <Image
                          key={`hero-${idx}`}
                          source={{ uri: img }}
                ...
                <View className="flex-row">
                  <Pressable
                    ...
                    className={`flex-1 py-2.5 rounded-lg mr-2 flex-row items-center justify-center ${
                      streamer.isLive ? "bg-red-600" : "bg-green-600"
                    }`}
                  >
            ...
            {streamers.length === 0 && (
              <View className="items-center py-12">
                ...
                <Text className="text-gray-600 text-sm">Create your first streamer to get started</Text>
              </View>
            )}
          </View>
        )}
                ...
                >
                  <View className="flex-row items-center justify-between mb-3">
                    ...
                    <View className={`px-2 py-1 rounded ${
                      accountType === "superfan" ? "bg-pink-600" :
                    ...
                    <View className="flex-row items-center justify-between">
                      ...
                      <View
                        className={`px-2 py-1 rounded ${
                          request.status === "approved" ? "bg-green-600/20" : "bg-red-600/20"
                        }`}
                      >
                        <Text
